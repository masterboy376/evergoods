    ...
    return (
        <>
            <div className='min-h-screen '>
                ...
                <p className="sm:text-7xl text-5xl text-gray-500 pb-10 font-thin">Headphones</p>
                        <div className="flex flex-wrap -m-4">
                            {
                                products.map((product) => {
                                    return <div key={product.slug} className="lg:w-1/4 md:w-1/2 w-full p-4">
                                        <Link href={`/product/${product.slug}`}>
                                            <div className="bg-white border-2 flex flex-col transition-all duration-300 hover:shadow ease-in-out justify-between rounded cursor-pointer w-full h-full">
                                                {/* wishlist button  */}
                                                <button onClick={(e) => {
                                                    e.stopPropagation()
