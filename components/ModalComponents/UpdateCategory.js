...
const UpdateCategory = () => {
    const { setUpdateCategoryModal, updateCategory, updateCategoryModal} = useContext(AdminContext)
    const [details, setDetails] = useState(updateCategoryModal.details)
    ...
    const onChange = (e)=>{
     ...
     const onSubmit = async (e)=>{
        e.preventDefault()
        console.log(updateCategoryModal.id, details)
        await updateCategory(updateCategoryModal.id, details)
      }
  ...
  return (
    <div className="w-11/12 bg-white mx-auto sm:w-3/4 h-auto p-4 px-4 max-w-5xl rounded border-2">
        ...
        <div className="flex w-full justify-between items-center p-2 mb-1 border-b">
            <span className="font-semibold sm:text-2xl text-xl ">Update Category</span>
            <button onClick={() => {setUpdateCategoryModal({state:false , details:{}, id:''})}} className='cursor-pointer'><GrClose /></button>
        </div>
