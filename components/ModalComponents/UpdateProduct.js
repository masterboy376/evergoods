...
const UpdateProduct = () => {
    const { setUpdateProductModal, updateProduct, updateProductModal} = useContext(AdminContext)
    const [details, setDetails] = useState(updateProductModal.details)
    ...
    const onChange = (e)=>{
     ...
     const onSubmit = async (e)=>{
        e.preventDefault()
        console.log(updateProductModal.id, details)
        await updateProduct(updateProductModal.id, details)
      }
  ...
  return (
    <div className="w-11/12 bg-white mx-auto sm:w-3/4 h-auto p-4 px-4 max-w-5xl rounded border-2">
        ...
        <div className="flex w-full justify-between items-center p-2 mb-1 border-b">
            <span className="font-semibold sm:text-2xl text-xl ">Update Product</span>
            <button onClick={() => {setUpdateProductModal({state:false , details:{}, id:''})}} className='cursor-pointer'><GrClose /></button>
        </div>
